...
export default function BlogDetailsNavigation({
  ...
  const nextBlog =
    currentIndex < blog_data.length - 1 ? blog_data[currentIndex + 1] : null;
      ...
      <div className="project-details-1-navigation d-flex justify-content-between align-items-center">
        {prevBlog ? (
          <Link
            ...
            href={`/blog-details/${prevBlog.slug}`}
          >
            <i style={{ color: "#0047AB" }} className="fa-sharp fa-regular fa-arrow-left"></i>
            <span style={{ fontFamily: "var(--tp-ff-gelder)", color: "#0047AB" }}>Prev</span>
          </Link>
        ) : (
        ...
        {nextBlog ? (
          <Link
            ...
            href={`/blog-details/${nextBlog.slug}`}
          >
            <span
              style={{ fontFamily: "var(--tp-ff-gelder)", color: "#0047AB" }}
            >
            ...
            <i style={{ color: "#0047AB" }}  className="fa-sharp fa-regular fa-arrow-right"></i>
          </Link>
        ) : (
          <span className="project-details-1-next disabled">
            <span style={{ fontFamily: "var(--tp-ff-gelder)", color: "#0047AB" }}>Next</span>
            <i style={{ color: "#0047AB" }} className="fa-sharp fa-regular fa-arrow-right"></i>
          </span>
        )}
      </div>
