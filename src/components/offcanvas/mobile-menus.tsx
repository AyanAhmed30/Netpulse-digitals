...
export default function MobileMenus() {
  ...
  const hasDropdown = (menu: typeof menu_data[0]) => {
    return menu.dropdown_menus || menu.pages_mega_menu || menu.portfolio_mega_menus;
  };

  const handleClick = (menu: typeof menu_data[0], e: React.MouseEvent) => {
    if (hasDropdown(menu)) {
      // Prevent navigation, only toggle
      e.preventDefault();
      if (navTitle === menu.title) {
        setNavTitle("");
      } else {
        setNavTitle(menu.title);
      }
    }
    // If no dropdown, let the link navigate normally (do nothing here)
  };
  ...
  return (
    <>
      <nav className="tp-main-menu-content">
        <ul>
          {menu_data.map((menu) => {
            const isDropdown = hasDropdown(menu);
            return (
              <li
                key={menu.id}
                className={`has-dropdown ${menu.home_menus || menu.portfolio_mega_menus ? "has-homemenu" : ""} ${
              ...
              >
                ...
                >
                  {menu.title}
                  {isDropdown && (
                    <button className="dropdown-toggle-btn" aria-label="Toggle submenu">
                    ...
                    <div className="tp-menu-fullwidth">
                      <div className="tp-megamenu-portfolio p-relative">
                        <div className="tp-megamenu-portfolio-banner">
                          <Image src={port_img} alt="port-img" style={{ height: 'auto', width: '100%' }} />
                        </div>
                        <div className="row gx-50">
